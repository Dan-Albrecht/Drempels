...
char  szTexPath[MAX_PATH] = "c:\\program files\\drempels";
...
{    
	...
	DWORD mainproc_start_time = GetTickCount();
                        ...
                        if (TryLockSurface(lpDDSVS[(intframe+1)%2], &ddsd2))
					    {
                            if (g_bGeissMode)
                            {
                                for (i=0; i<FXW2; i++)
                                {
                                    ...
                                    int offset = (i*4) + h*ddsd1.lPitch;
                                    unsigned char c = 192 + (rand() % 64);
                                    ((unsigned char *)ddsd1.lpSurface)[offset] = c;
                                    ((unsigned char *)ddsd2.lpSurface)[offset] = c;
                                    ((unsigned char *)ddsd1.lpSurface)[offset+1] = c;
                                    ((unsigned char *)ddsd2.lpSurface)[offset+1] = c;
                                    ((unsigned char *)ddsd1.lpSurface)[offset+2] = c;
                                    ((unsigned char *)ddsd2.lpSurface)[offset+2] = c;
                                    ((unsigned char *)ddsd1.lpSurface)[offset+3] = c;
                                    ((unsigned char *)ddsd2.lpSurface)[offset+3] = c;
                                }

                                for (i=0; i<10; i++)
                                for (j=0; j<10; j++)
                                {
                                    int offset = ((i+0.5f)*ddsd1.dwWidth/10*4) + ((j+0.5f)*ddsd1.dwHeight/10)*ddsd1.lPitch;
                                    unsigned char c = 192 + (rand() % 64);
                                    ((unsigned char *)ddsd1.lpSurface)[offset] = c;
                                    ((unsigned char *)ddsd2.lpSurface)[offset] = c;
                                    ((unsigned char *)ddsd1.lpSurface)[offset+1] = c;
                                    ...
                                    for (i=0; i<UVCELLSX-2; i+=2) 
	    						        BlitWarpNon256AndMix(cell[i][j], cell[i+2][j], cell[i][j+2], cell[i+2][j+2],
                                            ...
                                            FXH2,	
                                             ...
                                             FXH2,
                                            high_quality);
                            else if (TEX.texW[0]==256 && TEX.texH[0]==256)
						        for (j=0; j<UVCELLSY-2; j+=2)
                                    for (i=0; i<UVCELLSX-2; i+=2) 
